...
## Insira aqui a chave da sua API
...
- Obrigatoriamente adicione um título que esteja de acordo com o tema proposto.
- No primeiro parágrafo, você precisa apresentar o tema ao leitor. Você também precisa apresentá-los seu ponto de vista através de uma tese e pode utilizar algum tipo de repertório cultural (filme, livro, série).   
...
- A estrutura deve ser clara, um paragráfo abaixo do outro.  
Observações:  
...
- Não escreva a redação se o tema e a tese forem muito abrangentes, não especificando a posição.
Os respectivos critérios de matéria, tema proposto e tese são: {criterios}.

            """
        resposta = model.generate_content(prompt, generation_config = gemini.GenerationConfig (temperature=1))
        print(resposta)
